...
import { db } from "../db";
import { bookmarks, categories, settings } from "../db/schema";
...
export async function syncToObsidian(customPath?: string): Promise<ObsidianSyncResult> {
  // Determine target directory
  let vaultPath = customPath;
  if (!vaultPath) {
    const savedSetting = await db
      .select()
      .from(settings)
      .where(eq(settings.key, "obsidian_vault_path"));
    vaultPath = savedSetting[0]?.value;
  }

  if (!vaultPath) {
    vaultPath = "C:\\Users\\smetc\\Documents\\Obsidian Vault\\30-Kaynaklar\\X Yer İmlerim";
  }
  ...
  const categoryGroups = new Map<
    string,
    Array<{ fileName: string; title: string; author: string; date: string }>
  >();
  ...
  indexContent += `> MindMark-X tarafından otomatik senkronize edilmiştir. Son güncelleme: ${new Date().toLocaleString(
    "tr-TR"
  )}\n\n`;
  ...
  for (const [catName, items] of categoryGroups.entries()) {
    categorySummary.push({ name: catName, count: items.length });
  ...
  return {
    success: true,
    total: successfulBookmarks.length,
